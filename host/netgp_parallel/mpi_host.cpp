...
int main(int argc, char** argv) {
    ...
    std::cout << world_rank << " starting " << std::endl;
    log_info("[INFO] Processor %d start ... loading graph %s", world_rank, g_name.c_str());

    acceleratorSchedule(g_name, &graphDataInfo);
    log_info("[INFO] Processor %d load schedule file done %s", world_rank, g_name.c_str());
