...
template <typename T>
void  cuDuplicate_two ( int               loopNum,
                        hls::stream<T>    &input,
                        hls::stream<T>    &output_1,
                        hls::stream<T>    &output_2)
{
#pragma HLS function_instantiate variable=input
    for (int i = 0; i < loopNum ; i++)
    {
#pragma HLS PIPELINE II=1
        T  unit;
        read_from_stream(input, unit);
        write_to_stream(output_1, unit);
        write_to_stream(output_2, unit);
    }
}
...
void applyFunction(
    ...
    hls::stream<burst_dest>          &tmpVertexPropStream,
    unsigned int                    argReg,
    hls::stream<burst_dest>          &newVertexPropStream,
    int                             *outReg
)
{
...
#pragma HLS PIPELINE II=1
        ...
        burst_dest newVertexProp;

        for (int i = 0; i < BURST_ALL_BITS / INT_WIDTH; i++)
        {
#pragma HLS UNROLL
            prop_t tProp     = tmpVertexProp.data.range((i + 1) * INT_WIDTH - 1, i * INT_WIDTH );
...
//#pragma HLS DEPENDENCE variable=tmpInfoArray inter false
            ...
            newVertexProp.data.range((i + 1) * INT_WIDTH - 1, i * INT_WIDTH ) = wProp;
